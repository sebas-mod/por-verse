...
export async function engineCheck() {
    ...
    console.log(
        chalk.cyan.bold(`
╭───────────────────────────────╮
│ 🍓 WAGURI AI ENGINE CHECKER 🍰
│ ──────────────────────────────
│ 🍩 Versión       : ${chalk.blue(pkg.version)}
│ 🍰 Node.js       : ${nodeOk ? chalk.green("OK v" + nodeVersion) : chalk.red("FALLIDO (" + nodeVersion + ")")}
│ 🍬 FFmpeg        : ${ffmpegOk ? chalk.green("OK") : chalk.red("FALLIDO")}
│ 🍪 ImageMagick   : ${convertOk ? chalk.green("OK") : chalk.red("FALLIDO")}
│ 🥂 Git           : ${gitOk ? chalk.green("OK") : chalk.red("FALLIDO")}
│ 📦 Unzip & Zip   : ${unzipOk && zipOk ? chalk.green("OK") : !unzipOk && !zipOk ? chalk.red("FALLIDO") : unzipOk ? chalk.yellow("Unzip OK, Zip FALTANTE") : chalk.yellow("Zip OK, Unzip FALTANTE")}
│ 🥟 g++           : ${gppOk ? chalk.green("OK") : chalk.red("FALLIDO")}
│ 🥯 make          : ${makeOk ? chalk.green("OK") : chalk.red("FALLIDO")}
│ 🍱 Python3       : ${pyOk ? chalk.green("OK") : chalk.red("FALLIDO")}
│ 🎀 Número Pairing: ${pairingOk ? chalk.yellow(global.config.pairingNumber) : chalk.red("FALLIDO")}
╰───────────────────────────────╯
      © 2024 KenisawadevOlper
`)
    );
...
if (global.config?.DEVELOPER) {
    console.log(chalk.yellow("🛠️ Modo desarrollador activo → actualización automática omitida"));
    return;
}
