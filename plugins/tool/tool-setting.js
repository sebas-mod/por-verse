...
handler.before = async function (m, { conn }) {
    ...
    // Solo procesamos eventos importantes
    ...
    try {
        ...
        groupPic = global.db.data.settings?.welcomeImage || "https://n.uguu.se/qiUlhRsD.jpg";
    }

    try {
        // 📥 BIENVENIDA
        if (m.messageStubType === 27) {
            if (!chat.welcome) return; // <-- Verificación de activación
        ...
        // 📤 DESPEDIDA
            ...
            if (!chat.welcome) return; // <-- Opcional: controlar también la despedida
